      ...
      const killContainers = Promise.all(
        containers.map(async c => {
          try {
            ...
            buildLog(
              `Failed to stop <${c.alias || c.id}> container: ${
                e.message
              }\nForcefully killing...`,
            );
            try {
              ...
              buildLog(
                `Failed to kill <${c.alias || c.id}> container: ${err.message}`,
              );
            }
          }
        }),
      ...
      const killNetworks = Promise.all(
        Array.from(networks).map(async n => {
          try {
            ...
            buildLog(
              `Failed to delete network '${n}' (probably does not exist): ${err.message}`,
            );
          }
        }),
      );
...
export async function runDbContainer(
  ...
  if (dockerUrl && network) {
    buildLog(
      `Database accessible at ${dockerUrl} (on '${network}' docker network)`,
    );
  }
  const container = {
    alias,
